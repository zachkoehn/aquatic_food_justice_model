...
dat_clean <- dat_raw[-c(1,2),] %>% #note this also removes the first row that classifies gender and the second row specifying column number
  select('Country or territory','Reference year',educational_attainment_vars) %>% #selects only columns we want (convenient column also includes 'both' genders)
  ...
  transmute(educational_attainment_annual=sum(c_across(isced_3:isced_8),na.rm=TRUE)) %>% # sum across secondary & tertiary education levels, remove old variables
  group_by(country_name_en) %>% #now group by country and aggregate across year values
  summarize(mean_educ=mean(educational_attainment_annual,na.rm=TRUE)) # aggregate by mean
...
dat_final <- dat_clean %>%
  ...
  select(country_name_en,iso3c,iso3n,mean_educ,year_range) #only select variable 
...
write.csv(
  ...
  here(
    "data",
    "data_clean",
    "educ_attain_mean_2006-2016.csv"
  ),
  row.names = FALSE
)
