...
write.csv(fishstat_dat_total, file.path(outdir, "production_total_faostat_ts.csv"), row.names = FALSE, quote = FALSE)
...
fishstat_dat_by_source <- fishstat_dat %>%
  ...
  ungroup() %>%
  group_by(country_name_en, iso3n, iso3c, year, source_name_en, unit) %>% 
...
write.csv(fishstat_dat_by_source, file.path(outdir, "production_by_source_faostat_ts.csv"), row.names = FALSE, quote = FALSE)
